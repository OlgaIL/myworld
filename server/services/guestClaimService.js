...
export async function claimGuestDocumentForUser(req) {
  ...
  for (const guestDocument of guestDocuments) {
    ...
    const claimedPhoto = await createPhoto({
      ...
      formattedContent: guestDocument.formatted_content || { blocks: [] },
      formattedAt: guestDocument.formatted_at || null,
      hasTable: Boolean(guestDocument.has_table),
      hasFormulas: Boolean(guestDocument.has_formulas),
      hasRecognitionErrors: Boolean(guestDocument.has_recognition_errors),
      corrections: Array.isArray(guestDocument.corrections) ? guestDocument.corrections : [],
