...
export const YANDEX_RESPONSE_JSON_SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    formattedText: {
      type: "string",
      description: "Полный текст документа. Каждая строка начинается с H| для заголовка, P| для абзаца или поля, N| для нумерованного пункта, L| для маркированного пункта."
    },
...
- Если исправляешь слово, имя, число или другой лексический фрагмент, сразу добавь в corrections точную пару original из OCR и replacement из formattedText. Не включай изменения пробелов, регистра, пунктуации, переносов и структуры.

Структура formattedText:
- Это полный восстановленный текст, а не пересказ.
- Каждую самостоятельную часть возвращай с новой строки и обязательным префиксом: H| для явного заголовка, P| для абзаца, реквизита или поля, N| для каждого пункта нумерованного списка, L| для каждого пункта маркированного списка.
- В N| сохраняй исходный номер в начале текста, например N|2) Второй пункт. Сервер удалит его перед отображением и поставит правильную нумерацию сам.
- Не смешивай N| и L|: подпункты с маркерами после нумерованного пункта возвращай отдельными строками L|.
- Если OCR вынес последовательные номера в отдельные строки, связывай их со следующими пунктами только когда структура списка однозначна; после связывания не возвращай эти номера отдельным абзацем.
- Не объединяй весь многострочный OCR-текст в одну строку. Каждый самостоятельный заголовок, абзац, реквизит или поле бланка возвращай отдельной строкой.
- Сохраняй смысловые переносы исходного OCR. Объединяй соседние строки только при явном переносе одной незаконченной фразы, а не просто для сокращения числа блоков.
- В заявлениях, анкетах и других бланках короткие подписи полей вроде адреса, телефона, заявителя и госпошлины сохраняй отдельными строками P| в исходном порядке.
